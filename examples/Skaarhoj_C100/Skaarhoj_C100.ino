/*****************
 * Basis control for the SKAARHOJ C100 series devices
...
MenuBackend menu = MenuBackend(menuUseEvent,menuChangeEvent);
  ...
  MenuItem menu_network   = MenuItem(menu, "Network", 1);
    MenuItem menu_ownIP   = MenuItem(menu, "Device IP", 2);
    MenuItem menu_AtemIP  = MenuItem(menu, "ATEM IP", 2);
    MenuItem menu_ownMAC  = MenuItem(menu, "Device MAC", 2);
  MenuItem menu_maintenance   = MenuItem(menu, "Maintenance", 1);
    MenuItem menu_deviceUptInits   = MenuItem(menu, "Device Upt/Inits", 2);
    MenuItem menu_ATEMUptInits  = MenuItem(menu, "ATEM Uptim/Inits", 2);
    MenuItem menu_calibrateTbar  = MenuItem(menu, "Calibrate T-bar:", 2);
...
void menuSetup()
{
  ...
  menu.getRoot().add(menu_mediab1); 
    ...
    menu_network.addAfter(menu_maintenance);
    menu_maintenance.addAfter(menu_UP2);
    ...
    menu_AtemIP.addAfter(menu_ownMAC);    // Chain subitems...
    menu_AtemIP.addLeft(menu_network);      // Add parent item
    menu_ownMAC.addLeft(menu_network);      // Add parent item
    menu_network.addRight(menu_ownIP);     // Add the submenu to the parent

      // Set up maintenance:
    menu_deviceUptInits.addAfter(menu_ATEMUptInits);    // Chain subitems...
    menu_ATEMUptInits.addAfter(menu_calibrateTbar);    // Chain subitems...
    menu_ATEMUptInits.addLeft(menu_maintenance);      // Add parent item
    menu_calibrateTbar.addLeft(menu_maintenance);      // Add parent item
    menu_maintenance.addRight(menu_deviceUptInits);     // Add the submenu to the parent
}
...
void menuChangeEvent(MenuChangeEvent changed)
{
  ...
  if (changed.to.getName()=="MenuRoot")  {
    ...
    if (!(bool)changed.to.getRight())  {
        ...
        if (changed.to.getName() == menu_ownIP.getName())  { setMenuValues=31;  }
        if (changed.to.getName() == menu_AtemIP.getName())  { setMenuValues=32;  }
        if (changed.to.getName() == menu_ownMAC.getName())  { setMenuValues=33;  }
        if (changed.to.getName() == menu_deviceUptInits.getName())  { setMenuValues=41;  }
        if (changed.to.getName() == menu_ATEMUptInits.getName())  { setMenuValues=42;  }
        if (changed.to.getName() == menu_calibrateTbar.getName())  { setMenuValues=43;  }
...
unsigned long connectionUptime = 0;  // Reset everytime an ATEM connection is made
uint8_t connectionCount = 0;  // Counts up everytime an ATEM connection is made
  ...
  if (isConfigMode)  {
    ...
    if (AtemSwitcher.isConnectionTimedOut())  {
        if (AtemOnline)  {
          ...
          clearLCD();
          lcdPosition(0,0);
          LCD.print(F("Connection Lost!Reconnecting..."));
       ...
       AtemSwitcher.connect();
       connectionUptime = millis();  // Reset everytime an ATEM connection is made
    }
  
      // If the switcher has been initialized, check for button presses as reflect status of switcher in button lights:
    if (AtemSwitcher.hasInitialized())  {
      if (!AtemOnline)  {  // One time status:
        AtemOnline = true;
        connectionUptime = millis();  // Reset everytime an ATEM connection is made
        connectionCount++;    // Counts up everytime an ATEM connection is made
        menu.toRoot();
...
void menuValues()  {
      ...
      switch(setMenuValues)  {
        ...
        case 31:  // Own IP:
          lcdPosition(1,0);
          LCD << EEPROM.read(0+2) << "." << EEPROM.read(1+2) << "." << EEPROM.read(2+2) << "." << EEPROM.read(3+2);
        break;
        case 32:
          lcdPosition(1,0);
          LCD << EEPROM.read(0+2+4) << "." << EEPROM.read(1+2+4) << "." << EEPROM.read(2+2+4) << "." << EEPROM.read(3+2+4);
        break;
        case 33:
          lcdPosition(0,0);
          sprintf(buffer, "%02X:%02X:%02X", EEPROM.read(10), EEPROM.read(11), EEPROM.read(12));
          LCD << F("Device  ") << buffer;

          lcdPosition(1,0);
          sprintf(buffer, "%02X:%02X:%02X", EEPROM.read(13), EEPROM.read(14), EEPROM.read(15));
          LCD << F("MAC:    ") << buffer;
        break;
        case 41:
          if (encValue==1 || encValue==-1)  {
            level2State+=encValue;
            if (level2State>1) level2State=0;
          }
          lcdPosition(1,0);
          switch(level2State)  {
            case 1:
              LCD << F("Reset counters?");
              if (encValue>2 && encValue <1000)  {
                 level2State=0;
                 uptime = millis();
                 EEPROM.write(17,0);
            ...
            default:
              ...
              long mins;
              long secs;
              secs = (unsigned long)(millis()-uptime)/1000;
              mins = secs/60;
              hours = mins/60;
              secs = secs-(mins*60);
              mins = mins-(hours*60);
              sprintf(buffer, "%3d:%02d:%02d / %3d ", (int)hours, (int)mins, (int)secs, (int)EEPROM.read(17));
              LCD << buffer;
            break;
          }
        break;
        case 42:
          ...
          long mins;
          long secs;
          secs = (unsigned long)(millis()-connectionUptime)/1000;
          mins = secs/60;
          hours = mins/60;
          secs = secs-(mins*60);
          mins = mins-(hours*60);
              sprintf(buffer, "%3d:%02d:%02d / %3d ", (int)hours, (int)mins, (int)secs, (int)connectionCount);
          LCD << buffer;
        break;
        case 43:
          lcdPosition(1,0);
